<div class="py-6">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <h2 class="font-semibold text-xl text-gray-800">Reporte de ganancias</h2>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="bg-white shadow rounded-xl p-4">
                <div class="text-sm text-gray-500">Salidas en {{ $nombreMes }}</div>
                <div class="text-2xl font-extrabold text-gray-900 mt-1">{{ $salidas->count() }}</div>
            </div>
            <div class="bg-white shadow rounded-xl p-4">
                <div class="text-sm text-gray-500">Recuperado en el mes</div>
            ...
            <div class="bg-white shadow rounded-xl p-4 border-s-4 {{ $totalMes >= 0 ? 'border-green-500' : 'border-red-500' }}">
                <div class="text-sm text-gray-500">Ganancia de {{ $nombreMes }}</div>
            ...
            <div class="bg-white shadow rounded-xl p-4 border-s-4 {{ $acumulada >= 0 ? 'border-green-500' : 'border-red-500' }}">
                <div class="text-sm text-gray-500">Ganancia acumulada</div>
                <div class="text-2xl font-extrabold mt-1 {{ $acumulada >= 0 ? 'text-green-700' : 'text-red-700' }}">{{ dinero($acumulada) }}</div>
            </div>
        </div>

        <p class="text-sm text-gray-500">
            💼 Capital invertido en el inventario actual: <span class="font-semibold text-gray-800">{{ dinero($invertidoInventario) }}</span>
        </p>
                ...
                @forelse ($salidas as $salida)
                    <a href="{{ route('vehiculos.ficha', $salida['vehiculo']) }}" class="block p-4">
                        ...
                        <div class="text-xs text-gray-500 mt-1">
                            {{ $salida['tipo'] }} · {{ $salida['fecha']->format('d/m/Y') }} ·
                            compra {{ dinero($salida['compra']) }} · gastos {{ dinero($salida['gastos']) }} · recibido {{ dinero($salida['recuperado']) }}
                        </div>
                    </a>
                @empty
                    <p class="p-6 text-center text-gray-500 text-sm">Sin ventas ni desguaces en {{ $nombreMes }}.</p>
                @endforelse
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
                            ...
                            <th class="px-4 py-3 text-right">Recuperado</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center text-gray-500">Sin ventas ni desguaces en {{ $nombreMes }}.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if ($salidas->isNotEmpty())
                        <tfoot class="bg-gray-50 font-semibold">
                            <tr>
                                <td class="px-4 py-3" colspan="3">Totales de {{ $nombreMes }}</td>
