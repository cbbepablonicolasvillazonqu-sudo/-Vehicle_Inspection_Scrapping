<div>
    @if ($venta)
        ...
        <div class="bg-white shadow rounded-xl p-4 sm:p-6 border-t-4 border-blue-600">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <h3 class="text-lg font-semibold text-gray-800">Venta 🔵</h3>
                ...
                @if ($this->puedeEditarCerrada() && ! $editando)
                    <div class="flex gap-2">
                        ...
                        <button wire:click="eliminarVenta"
                                wire:confirm="¿Eliminar la venta? El vehículo volverá a su estado anterior."
            ...
            @if (! $editando)
                <dl class="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 text-sm">
                    ...
                    <div>
                        <dt class="text-gray-500">Registrada por</dt>
                        ...
                        @if (! is_null($vehiculo->ganancia()))
                            <div class="col-span-2 sm:col-span-3 mt-1 bg-gray-50 border border-gray-200 rounded-lg p-3">
                                ...
                                <span class="text-xs text-gray-400 block sm:inline sm:ms-2">
                                    (venta {{ dinero($venta->precio_venta) }} − compra {{ dinero($vehiculo->precio_compra) }} − gastos {{ dinero($vehiculo->totalGastos()) }})
                                </span>
                            </div>
                        @endif
                    @endcan
    ...
    @elseif ($this->puedeVender())
        {{-- Formulario de venta --}}
        <div class="bg-white shadow rounded-xl p-4 sm:p-6 border-t-4 border-blue-600">
            <h3 class="text-lg font-semibold text-gray-800">Registrar venta</h3>
            <p class="mt-1 text-sm text-gray-500">
                Al guardar, el vehículo pasará a <span class="font-semibold text-blue-700">Vendido</span> y el registro quedará bloqueado (solo Admin podrá editarlo).
            </p>
            ...
            <form wire:submit="registrar" class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2 flex justify-end">
                    <button type="submit"
                            wire:confirm="¿Confirmar la venta? El registro quedará bloqueado."
                            class="px-6 py-3 bg-blue-700 hover:bg-blue-800 text-white text-base font-semibold rounded-xl shadow"
                            wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="registrar">Marcar como Vendido</span>
